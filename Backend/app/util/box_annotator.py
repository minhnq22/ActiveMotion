...
def IoU(box1, box2, return_max=True):
    ...
    Args:
        ...
        return_max: If True, return max of IoU and intersection ratios
    ...
    if return_max:
        return max(intersection / union, ratio1, ratio2) if union > 0 else 0
    else:
...
def get_optimal_label_pos(text_padding, text_width, text_height, x1, y1, x2, y2, detections, image_size):
    """
    Check overlap of text and background detection box, and return optimal label position.
    
    Tries positions in order: top left, outer left, outer right, top right.
    If all positions overlap, returns the last one (outer right).
    Threshold for overlap detection: 0.3
    """
    ...
    # Try top right position (last resort)
